...
export class CmdPacket {
  ...
  public decode(data: number[]): number {
    ...
    let s = 0;
    // Sum HEAD1 + HEAD2 + ADDR + CMD + LEN + all DATA bytes
    // Note: original NanoKVM-USB uses headerIndex + 4 + dataLen which misses the last
    // data byte. Fixed here to headerIndex + 5 + dataLen for correctness.
    for (let i = headerIndex; i < headerIndex + 5 + dataLen; i++) {
      s += data[i];
    }
